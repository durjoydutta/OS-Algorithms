...
void scanDiskSchedule(int start, vector<int> references, string &dir, int lower, int upper)
{
    ...
    if (dir == "left")
    {
        ...
        schedule(currPos, references, startPos + 1, high - 1, 1);
    }
    else if (dir == "right")
    {
        schedule(currPos, references, startPos + 1, high, 1);
        schedule(currPos, references, 1, startPos - 1, -1);
    }
...
int main()
{
    ...
    scanDiskSchedule(start, references, dir, lower, upper);
